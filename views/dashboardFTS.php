...
<html lang="en">
	...
	<body>
		...
		<script>
			alert("Welcome <?php echo ($this->session->userdata('DeptName'));?>"+"\n"+" The number of pending files <?php  
			  $this->load->database();
      $this->db->where("status",0);
      $this->db->where("mark.receiver_id",$this->session->userdata('DeptId'));
      $num_rows = $this->db->count_all_results('mark');
      echo ($num_rows);?>
      ");
		</script>
		<script>
			alert("Welcome <?php echo ($this->session->userdata('DeptName'));?>"+"\n"+" File handling efficiency <?php  
			  $this->load->database();
      $this->db->where("status",0);
      $this->db->where("mark.receiver_id",$this->session->userdata('DeptId'));
      $num_rows1 = $this->db->count_all_results('mark');
      $this->load->database();
      $this->db->where("status",1);
      $this->db->where("mark.receiver_id",$this->session->userdata('DeptId'));
      $num_rows2 = $this->db->count_all_results('mark');
      $num_rows3=(($num_rows2/($num_rows1+$num_rows2))*100);
      $num_rows4=sprintf("%.2f", $num_rows3);
      echo ($num_rows4);?>%");
		</script>
		<div id="app">		
