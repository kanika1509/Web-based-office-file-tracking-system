...
class TrackPageController2 extends CI_Controller {
	public function index()
	{
		   
		 $this->load->helper('url');
		$this->load->view('TrackPageView2');
			}
	public function process()
	{
		...
		$this->load->model('TrackPageModel2');
		$filedetails=$this->TrackPageModel2->FileDetails($file_id);
		$res1['FileDetails']=$filedetails;
		$var=sizeof($filedetails);
		$counter=0;
if($var!=0)
{$this->load->helper('url');
    		$this->load->view('FileRecordView2',$res1);
    	}
    	else if($var==0)
    	{
    		$this->load->helper('url');
    		$data['error']="This file does not exist in database Please renter your File Id ";
    		$this->load->view('TrackPageView2',$data);
    	}
